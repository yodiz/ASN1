...
module internal Parsers = 
    ...
    let pSequence =
        ...
        .>> ws .>> pOptionalExtensionMarker .>> ws .>> pstringCI "}" .>> ws


    let pChoice = 
        pstringCI "CHOICE" .>> ws .>> pstringCI "{" .>> ws
        >>. (sepEndBy pNamedType (pchar ',' .>> ws))
        .>> ws .>> pOptionalExtensionMarker .>> pstringCI "}" .>> ws 
...
let asn1Module = Parsers.pModuleDefinition
